/* Combos leaderboard — daily highest-score rankings, backed by Firestore.
 *
 ...
 *   - configured     true once Firebase initialised from window.CombosFirebaseConfig
 ...
 *
 * Scores live in their own collection (combos_scores), keyed by
 * `${date}_${userId}`, so there's one row per player per day and Combos never
...
const COLLECTION = "combos_scores";

let db = null;
const cfg = window.CombosFirebaseConfig;
if (cfg && cfg.apiKey) {
  try { db = getFirestore(initializeApp(cfg)); }
  catch (e) { console.warn("Combos leaderboard: Firebase init failed", e); }
}
...
async function submitScore({ userId, name, date, score }) {
  ...
  try {
    ...
    console.warn("Combos leaderboard: submit failed", e);
  }
}
...
async function fetchBoard(date) {
  ...
  try {
    ...
    console.warn("Combos leaderboard: fetch failed", e);
    return null;
  }
}
